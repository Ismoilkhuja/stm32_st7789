...
import qbs
import "qbs/stm32f4.qbs" as stm32f4
...
Project {
     ...
     * - STM32F423xx
     */
    readonly property string microcontroller_series: "STM32F401xC"
    ...
    readonly property bool   fpu        : true  // use hardware floating-point unit
    ...
    readonly property bool   hal        : false  // use ST's HAL library
    readonly property bool   ll         : false  // use ST's LL library
    ...
    readonly property string stemwin_v  : "540" // ST's STemWin library version
    readonly property bool   freertos   : false // use the FreeRTOS real-time operating system
    readonly property bool   mpu        : false // use the MPU in FreeRTOS
    ...
    stm32f4 {
        ...
        property string appPath       : basePath + "/Core"
        property string startupPath   : basePath
        property string cmsisPath     : basePath + "/Drivers/CMSIS"
        property string halPath       : basePath + "/Drivers/STM32F4xx_HAL_Driver"
        ...
        cpp.includePaths: {
            ...
            if (project.cmsis) {
                ...
                includePaths.push(cmsisPath + "/Device/ST/STM32F4xx/Include")
            }
            ...
            if (project.freertos) {
                ...
                if (project.mpu) {
                    includePaths.push(freertosPath + "/Source/portable/GCC/ARM_CM4_MPU")
                } else {
                    includePaths.push(freertosPath + "/Source/portable/GCC/ARM_CM4F")
                }
            }
        ...
        cpp.staticLibraries: {
            ...
            if (project.cmsis_math) {
                if (project.fpu) {
                    staticLibraries.push("arm_cortexM4lf_math")
                } else {
                    staticLibraries.push("arm_cortexM4l_math")
                }
            }

            if (project.audio) {
                if (project.fpu) {
                    staticLibraries.push("PDMFilter_CM4F_GCC")
                } else {
                    staticLibraries.push("PDMFilter_CM4_GCC")
                }
            }
            ...
            if (project.stemwin) {
                if (project.freertos) {
                    if (qbs.buildVariant === "release") {
                        staticLibraries.push("STemWin" + project.stemwin_v + "_CM4_OS_GCC_ot")
                    } else {
                        staticLibraries.push("STemWin" + project.stemwin_v + "_CM4_OS_GCC")
                    }
                } else {
                    if (qbs.buildVariant === "release") {
                        staticLibraries.push("STemWin" + project.stemwin_v + "_CM4_GCC_ot")
                    } else {
                        staticLibraries.push("STemWin" + project.stemwin_v + "_CM4_GCC")
                    }
                }
            }
